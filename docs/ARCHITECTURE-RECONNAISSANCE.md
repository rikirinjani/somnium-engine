...
**Five routes around the same wall, found by five separate passes.** P-003 built the `declared` gate so that absence of knowledge could not become fact. Each of these walked around it:
...
> **Only canon declares an occurrence.** Canon defines the vocabulary of a world; an intervention selects among it. Two invariants enforce that, each at a point where every route converges. **Truth:** an id canon never declared has `truth: FALSE`, and no conflict about it may lift that truth — a predicate on the node, not a list of conflict kinds — so no intervention and no composition of interventions can make it occur, be counted, open or close a fact window, satisfy a `WorkBinding.facts` requirement, change a `workStatus`, or contribute a `temporalViolation`. **Assertion:** a fact's subject must be something canon declares, so no intervention can write a fact about an undeclared id — including enrolling one in an `EventType` via `instance_of`. Incoherent interventions are reported (`contra:<id>:force-undeclared`, `contra:<id>:fact-write-undeclared-subject`) with the intervention as provenance, never silently dropped. Such an id may still appear as a node — an `addEdge` endpoint puts one in the node set — but it is `UNKNOWN`, which is the honest report for something canon does not mention.
