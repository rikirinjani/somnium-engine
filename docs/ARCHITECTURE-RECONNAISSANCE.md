...
**Four routes around the same wall, found by four separate passes.** P-003 built the `declared` gate so that absence of knowledge could not become fact. Each of these walked around it:
...
Sequence, because the pattern matters far more than any single hole. Pass 1 fixed (1) and asserted in prose that "every other route to an undeclared id already respected that gate". Pass 2 fixed (2) and (3) and restated the rule. Pass 3 fixed the `BOTH` path and restated it again. Each restatement was a universal quantified over interventions; each was refuted by the next gate; each refutation was narrower than the last. **Four false universals in a row, on the change's own headline rule.** The unifying error was not any individual oversight but the shape of the claim: a universal over an open set ("any intervention") was defended by enumerating members of that set. The invariant is now enforced at the one place all routes converge — the node's truth value — and tested by quantifying over the intervention vocabulary rather than over a hand-picked route list. Recorded as ncr-004.
...
> **Only canon declares an occurrence.** Canon defines the vocabulary of a world; an intervention selects among it. An id canon never declared has `truth: FALSE` — the world does not contain it — and **no conflict about it may lift that truth**, which is enforced as a predicate on the node rather than as a list of conflict kinds. So no intervention and no composition of interventions can put it into the world: not forcing it, not adding an edge to it, not enabling it, not forcing-and-negating it together. Forcing one is an incoherent intervention, reported as `contra:<id>:force-undeclared` with the forcing intervention as provenance. Such an id may still appear as a node — an `addEdge` endpoint puts one in the node set — but it can never occur, be counted, join an `EventType`, or open or close a fact window.
...
- **No occurrence can be manufactured.** Be precise about what is guaranteed, because three earlier drafts of this bullet were false. The node *set* is `canon events ∪ edge endpoints ∪ intervention targets`, and `addEdge` **can** put a fresh id in it — so "nothing adds a node" is wrong. What holds is the claim that matters: **an id canon never declared has `truth: FALSE`, and nothing can lift it.** Enforced at two levels. Derivation: `hardSupport` returns `NEITHER` for any undeclared node however many edges point at it; `softSupported` refuses it `SOFT` support; the forced branch of `positiveFixpoint` yields `FALSE` rather than `TRUE`. Conflict resolution: `cannotBeTainted(node)` excludes every undeclared id from the taints-truth set, so no conflict of any kind — including compositions like `negateEvent` + `forceEvent` — can join it to `BOTH` (which `occurs()` would accept). An undeclared id may appear as an `UNKNOWN` or `UNSUPPORTED` node, the honest report for something canon does not mention; it can never be `ESTABLISHED`, be counted by `occurrenceCount`, join an `EventType`, or open or close the validity window of a canon fact. Nothing creates a node *during* derivation, so no fixpoint pass can grow the world. Quantified over the whole intervention vocabulary in `src/query/occurrence.test.ts`, each kind alone and composed with `forceEvent`.
...
> Only canon declares an occurrence. Canon defines the vocabulary; an intervention selects among it. An id canon never declared has `truth: FALSE` and cannot occur. Derivation denies it truth (`hardSupport` however many edges point at it, `softSupported` for soft support, the forced branch yielding `FALSE`), and conflict resolution denies it taint (`cannotBeTainted` covers every undeclared node, so no conflict kind and no composition of interventions can join it to `BOTH`). Forcing one yields `contra:<id>:force-undeclared` carrying the forcing intervention as provenance. Such an id may still appear as an `UNKNOWN` or `UNSUPPORTED` node — the honest report for something canon does not mention — but it can never be counted, join an `EventType`, or open or close a fact window.
