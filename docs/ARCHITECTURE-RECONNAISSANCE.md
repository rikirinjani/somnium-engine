...
**Six routes around the same wall.** P-003 built the `declared` gate so that absence of knowledge could not become fact. Each of these walked around it:
...
Both ends are now decided by a single exported predicate, `factWriteRejection(subject, object, declaredSubjects)`, called by `buildModel` and `applyFactInterventions` alike. One predicate, two call sites: the two views of a fact are now incapable of disagreeing, which is the same convergence-point argument as `cannotBeTainted` rather than another enumerated case.

Sequence, because the pattern matters far more than any single hole. Pass 1 fixed (1) and asserted in prose that "every other route to an undeclared id already respected that gate". Pass 2 fixed (2) and (3) and restated the rule. Pass 3 fixed the `BOTH` path and restated it again. Pass 4 made the guard node-level and restated it again. Pass 5 gated fact-write subjects — and pass 6, the object end, I found myself by probing pass 5 instead of asserting it. Each restatement before that was a universal quantified over interventions; each was refuted by the next gate; each refutation was narrower than the last. **Five false universals in a row, on the change's own headline rule.**

The unifying error was never any individual oversight. It was the shape of the claim: a universal over an open set ("any intervention") defended by enumerating members of that set. Enumeration cannot close such a universal, because the set grows — new conflict kinds, new intervention kinds, and decisively *compositions* of existing ones. What finally worked was different in kind: enforce the rule at each point where all routes converge, and state the rule as what those points guarantee rather than as a list of what cannot happen. There are two such points, because there are two things worth protecting — a node's truth (`cannotBeTainted`, covering occurrence, counting, fact windows, work statuses, temporal violations) and a fact's subject (`declaredSubjects`, covering type membership and every other assertion about an id). Recorded as ncr-004.
...
> **Only canon declares an occurrence.** Canon defines the vocabulary of a world; an intervention selects among it. Two invariants enforce that, each at a point where every route converges. **Truth:** an id canon never declared has `truth: FALSE`, and no conflict about it may lift that truth — a predicate on the node, not a list of conflict kinds — so no intervention and no composition of interventions can make it occur, be counted, open or close a fact window, satisfy a `WorkBinding.facts` requirement, change a `workStatus`, or contribute a `temporalViolation`. **Assertion:** both ends of a fact write must resolve — the subject must be something canon declares, and an id-shaped object must too — so no intervention can write a fact about an undeclared id, enrol one in an `EventType`, or invent the `EventType` itself. One exported predicate (`factWriteRejection`) decides this for both the causal graph and the effective-fact list, so the two cannot diverge. Incoherent interventions are reported (`contra:<id>:force-undeclared`, `:fact-write-undeclared-subject`, `:fact-write-undeclared-object`) with the intervention as provenance, never silently dropped. Such an id may still appear as a node — an `addEdge` endpoint puts one in the node set — but it is `UNKNOWN`, which is the honest report for something canon does not mention.
...
- **No occurrence can be manufactured.** Be precise about what is guaranteed, because four earlier drafts of this bullet were false. The node *set* is `canon events ∪ edge endpoints ∪ intervention targets`, and `addEdge` **can** put a fresh id in it — so "nothing adds a node" is wrong. What holds are two invariants, each at a convergence point. **Truth:** an id canon never declared has `truth: FALSE` and nothing can lift it — derivation denies it truth (`hardSupport` however many edges point at it, `softSupported` for soft support, the forced branch yielding `FALSE`) and conflict resolution denies it taint (`cannotBeTainted` covers every undeclared node, so no conflict kind and no composition of interventions can join it to `BOTH`, which `occurs()` would accept). Everything truth-derived follows: it can never be `ESTABLISHED`, be counted by `occurrenceCount`, appear in `diffOccurrences`' occurred sets, open or close a canon fact's validity window, satisfy a `WorkBinding.facts` requirement, change a `workStatus`, or contribute a `temporalViolation`. **Assertion:** `factWriteRejection` gates BOTH ends of a fact write (subject, and an id-shaped object) in BOTH views that own them (`buildModel`'s `overriddenCells` and `applyFactInterventions`' effective-fact list), so an undeclared id cannot join an `EventType`, cannot have an `EventType` invented for it, and cannot carry any other asserted fact. Documented limitation, shared with `validateCanon`: a literal string containing `/` is rejected as if it were a reference. An undeclared id may appear as an `UNKNOWN` or `UNSUPPORTED` node — the honest report for something canon does not mention. Nothing creates a node *during* derivation, so no fixpoint pass can grow the world. Both invariants are quantified over the whole intervention vocabulary in `src/query/occurrence.test.ts`, each kind alone and composed with `forceEvent`.
...
> Only canon declares an occurrence. Canon defines the vocabulary; an intervention selects among it. Two invariants, two convergence points. **Truth:** an id canon never declared has `truth: FALSE` and cannot occur — derivation denies it truth, and `cannotBeTainted` denies it taint for every conflict kind and every composition of interventions. **Assertion:** `factWriteRejection` denies a fact write naming it at either end, in both views that own the effective-fact list, so it cannot join an `EventType`, be invented as one, or carry any asserted fact. Incoherent interventions are reported with provenance (`contra:<id>:force-undeclared`, `contra:<id>:fact-write-undeclared-subject`), never silently dropped. Such an id may still appear as an `UNKNOWN` or `UNSUPPORTED` node — the honest report for something canon does not mention.
