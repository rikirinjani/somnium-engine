...
**Three independent holes in the same wall.** P-003 built the `declared` gate so that absence of knowledge could not become fact. Each of these walked around it, and each was found by a separate pass:
...
Sequence, because the pattern matters more than any single hole: pass 1 fixed (1) and asserted in prose that "every other route to an undeclared id already respected that gate" — false in the target direction. The second L2 gate found (2) and (3) by trying to *defeat* the claim rather than read it. The third gate found that pass 2's own restated rule was still false, via the `BOTH`/`occurs()` path. Three gates, three false universals, each one narrower than the last. Recorded as ncr-004.
...
> **Only canon declares an occurrence.** Canon defines the vocabulary of a world; an intervention selects among it. An id canon never declared has `truth: FALSE` — the world does not contain it — no matter what an intervention says: not by being forced, not by having an edge added to it, not by being enabled. Forcing one is an incoherent intervention, reported as a contradiction record with the forcing intervention as its provenance. Such an id may still appear as a node (an `addEdge` endpoint puts one in the node set), but it can never occur, be counted, join an `EventType`, or open a fact window.
...
- **No occurrence can be manufactured.** Be precise about what is guaranteed, because two earlier drafts of this bullet were false. The node *set* is `canon events ∪ edge endpoints ∪ intervention targets`, and `addEdge` **can** put a fresh id in it — so "nothing adds a node" is wrong. What holds is the claim that matters: **an id canon never declared has `truth: FALSE` and therefore cannot occur.** Four gates enforce it — `hardSupport` returns `NEITHER` for any undeclared node however many edges point at it; `softSupported` refuses it `SOFT` support; `forceEvent` on it yields `FALSE` plus a `forced-undeclared` contradiction record; and that record is deliberately excluded from the conflict-taints-truth set, so it cannot be joined to `BOTH` (which `occurs()` would accept). An undeclared id may appear as an `UNKNOWN` or `UNSUPPORTED` node, which is the honest report for something canon does not mention; it can never be `ESTABLISHED`, be counted by `occurrenceCount`, join an `EventType`, or open the validity window of a canon fact. Nothing creates a node *during* derivation, so no fixpoint pass can grow the world.
...
> Only canon declares an occurrence. Canon defines the vocabulary; an intervention selects among it. An id canon never declared has `truth: FALSE` and cannot occur: `hardSupport` denies it truth however many edges point at it, `softSupported` denies it soft support, `forceEvent` on it yields `FALSE` plus `contra:<id>:force-undeclared` carrying the forcing intervention as provenance, and that record cannot taint truth to `BOTH`. Such an id may still appear as an `UNKNOWN` or `UNSUPPORTED` node — the honest report for something canon does not mention — but it can never be counted, join an `EventType`, or open a fact window.
