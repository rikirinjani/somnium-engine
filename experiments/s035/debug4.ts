...
function compareModels(a: any, b: any): string[] {
  const m: string[] = [];
  if (a.nodeIds.length !== b.nodeIds.length) m.push("nodeIds.len=" + a.nodeIds.length + " vs " + b.nodeIds.length);
  else { for (let i = 0; i < a.nodeIds.length; i++) { if (a.nodeIds[i] !== b.nodeIds[i]) { m.push("nodeIds[" + i + "]=" + a.nodeIds[i] + " vs " + b.nodeIds[i]); break; } } }
  ...
  if (a.declared.size !== b.declared.size) m.push("declared.size=" + a.declared.size + " vs " + b.declared.size);
  return m;
}

// Reproduce lifecycle rewind test — EXACT code from probe.ts
...
if (diffs.length === 0) {
  ...
  console.log("FAIL:", diffs.join(", "));
  
  // Find which intervention caused the divergence
  // Rebuild both paths prefix by prefix and find first difference
  let pathA: Exec = { model: buildModel(canon, []), truth: phaseATruth(buildModel(canon, [])) };
  let pathB_model = restore(canon, checkpoint);
  
  for (let i = 10; i < 20; i++) {
    pathA = stepIncremental(canon, pathA, h[i]!);
    pathB_model = incrementalApply(canon, pathB_model, h[i]!);
    
    // Compare at each step
    const stepDiffs: string[] = [];
    if (pathA.model.edges.length !== pathB_model.edges.length) stepDiffs.push("edges.len=" + pathA.model.edges.length + " vs " + pathB_model.edges.length);
    if (pathA.model.negated.size !== pathB_model.negated.size) stepDiffs.push("negated.size");
    if (pathA.model.forcedBy.size !== pathB_model.forcedBy.size) stepDiffs.push("forcedBy.size");
    if (pathA.model.overriddenCells.size !== pathB_model.overriddenCells.size) stepDiffs.push("overriddenCells.size");
    
    if (stepDiffs.length > 0) {
      console.log(`  First divergence at prefix ${i + 1}: ${h[i]!.kind} target=${h[i]!.target}`);
      console.log(`  Diffs: ${stepDiffs.join(", ")}`);
      
      // Print edges detail
      if (pathA.model.edges.length !== pathB_model.edges.length) {
        console.log(`  pathA edges: ${pathA.model.edges.length}, pathB edges: ${pathB_model.edges.length}`);
        // Find missing/extra edges
        const aIds = new Set(pathA.model.edges.map((e: any) => e.id));
        const bIds = new Set(pathB_model.edges.map((e: any) => e.id));
        for (const id of aIds) if (!bIds.has(id)) console.log(`  In pathA but not pathB: ${id}`);
        for (const id of bIds) if (!aIds.has(id)) console.log(`  In pathB but not pathA: ${id}`);
      }
      
      // Print the intervention details
      console.log(`  Intervention: ${JSON.stringify(h[i]!).slice(0, 200)}`);
      break;
    }
  }
}
